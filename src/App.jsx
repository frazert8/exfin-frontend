...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-datarails-wood flex font-sans text-datarails-midnight">
      <motion.aside initial={false} animate={{ width: isSidebarOpen ? '16rem' : '5rem' }} transition={{ duration: 0.3, ease: "easeInOut" }} className="bg-white text-datarails-midnight relative flex flex-col flex-shrink-0 border-r border-gray-200">
            ...
            <AnimatePresence>
              {isSidebarOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.1 } }} exit={{ opacity: 0, transition: { duration: 0.1} }}>
                  <div className="flex items-center gap-3 font-bold text-xl">{icons.rocket} <span>ExFin.ai</span></div>
                </motion.div>
              )}
            </AnimatePresence>
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1 rounded-full hover:bg-gray-100">
              <div className={`text-gray-500 transition-transform duration-500 ${isSidebarOpen ? '' : 'rotate-180'}`}>{icons.chevronsLeft}</div>
            </button>
          </div>
          <nav className="px-4">
            <ul>
              <li><button onClick={() => setView('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-semibold transition-colors ${isSidebarOpen ? '' : 'justify-center'} ${view === 'dashboard' ? 'bg-datarails-red text-white' : 'text-gray-500 hover:bg-gray-100'}`}>{icons.dashboard} {isSidebarOpen && <span className="truncate">Dashboard</span>}</button></li>
              <li><button onClick={() => setView('admin')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors ${isSidebarOpen ? '' : 'justify-center'} ${view === 'admin' ? 'bg-datarails-red text-white' : 'text-gray-500 hover:bg-gray-100'}`}>{icons.lock} {isSidebarOpen && <span className="truncate">Admin</span>}</button></li>
            </ul>
          </nav>
        </div>
      </motion.aside>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-white/80 backdrop-blur-sm border-b border-gray-200 px-10 flex items-center justify-between flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="text-gray-500">{icons.search}</div>
            <div className="text-gray-500">{icons.bell}</div>
          </div>
        </header>

        <main className="flex-1 overflow-y-auto p-10">
          <AnimatePresence mode="wait">
            {loading ? ( <div key="loader" className="text-center text-gray-500">Loading...</div> ) : (
              <motion.div key={view} variants={pageMotionVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.2 }}>
                {view === 'dashboard' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <KPICard title="Total Revenue (YTD)" value={formatCurrency(totalRev)} trend="+12.4%" icon={icons.dollar} />
                    <KPICard title="Net Margin" value={`${margin.toFixed(1)}%`} trend="+2.1%" icon={icons.activity} />
                    <KPICard title="Cash on Hand" value={formatCurrency(latest.cashOnHand || 0)} trend="+0.4%" icon={icons.wallet} />
                    <KPICard title="Total Headcount" value={latest.headcount || 0} trend="+3" icon={icons.users} />
                  </div>
                )}
                {view === 'admin' && (
                  <div className="max-w-xl mx-auto">
                    <Card>
                      ...
                      <button onClick={handleSeedDatabase} disabled={source === 'supabase' || loading} className={`w-full py-2 rounded-lg font-semibold transition-colors ${source === 'supabase' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-datarails-red text-white hover:bg-datarails-red/90'}`}>
