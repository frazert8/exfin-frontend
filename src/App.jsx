...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-zinc-100 flex font-sans text-zinc-800">
      <motion.aside 
        initial={false}
        animate={{ width: isSidebarOpen ? '16rem' : '5rem' }}
        transition={{ duration: 0.3, ease: "easeInOut" }}
        className="bg-zinc-800 text-white relative flex flex-col flex-shrink-0"
      >
        <div className="h-full flex flex-col overflow-hidden">
          <div className="p-4 h-20 flex items-center justify-between flex-shrink-0">
            ...
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-1 rounded-full hover:bg-zinc-700">
          ...
          <nav className="px-4">
            <ul>
              <li><button onClick={() => setView('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-semibold transition-colors ${isSidebarOpen ? '' : 'justify-center'} ${view === 'dashboard' ? 'bg-zinc-900' : 'text-zinc-400 hover:bg-zinc-700'}`}><LayoutDashboard size={20} />{isSidebarOpen && <span className="truncate">Dashboard</span>}</button></li>
              <li><button onClick={() => setView('admin')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors ${isSidebarOpen ? '' : 'justify-center'} ${view === 'admin' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:bg-zinc-700'}`}><Lock size={20} />{isSidebarOpen && <span className="truncate">Admin</span>}</button></li>
            </ul>
          </nav>
        </div>
      </motion.aside>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <header className="h-20 bg-white border-b border-zinc-200 px-10 flex items-center justify-between flex-shrink-0">
          <div>
            <h2 className="text-2xl font-bold text-zinc-800 capitalize">{view}</h2>
            <p className="text-zinc-500 text-sm">Here is your overview.</p>
          </div>
        ...
        <main className="flex-1 overflow-y-auto p-10">
          <AnimatePresence mode="wait">
            {loading ? (
               <div key="loader" className="text-center text-zinc-500">Loading...</div>
            ) : (
                ...
                {view === 'admin' && (
                  <div className="max-w-xl mx-auto">
                    <Card>
                      ...
                      <p className="text-zinc-500 text-sm mb-4">If you are running this locally, you can seed the database with mock data.</p>
                      <button onClick={handleSeedDatabase} disabled={source === 'supabase' || loading} className={`w-full py-2 rounded-lg font-semibold transition-colors ${source === 'supabase' ? 'bg-zinc-100 text-zinc-400 cursor-not-allowed' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
